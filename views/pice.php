<div class="container" style="margin-top: 30px">
    <div class="row">
        ...
        $upitHrana = "SELECT * FROM proizvodi WHERE vrsta = 'pice'";
        $prepare = $konekcija->prepare($upitHrana);
        $prepare->execute();
        $brojRedova = $prepare->rowCount();
        $brStrana = ceil($brojRedova / 8);

        if(isset($_SESSION['upitPice'])):
            $upitCena = $_SESSION['upitPice'];
...
<div class="container">
    <?php for($i=1; $i<=$brStrana; $i++):   ?>
        <a class="btn btn-info" href="modules/cenovnikPice.php?brStrane=<?=$i?>"><?=$i?></a>
    <?php endfor; ?>
</div>